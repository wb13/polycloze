...
export function createPagination(
  ...
  const children: Array<string | Element> = [
    createPreviousButton(page === 1, () => gotoPage(page - 1)),
  ];
  ...
  if (lastPage - page >= 4) {
    ...
    children.push(
      createPageButton(lastPage - 1, page === lastPage - 1, () =>
        gotoPage(lastPage - 1)
      )
    );
  }
  if (lastPage - page >= 2) {
    children.push(
      createPageButton(lastPage, page === lastPage, () => gotoPage(lastPage))
    );
  }
